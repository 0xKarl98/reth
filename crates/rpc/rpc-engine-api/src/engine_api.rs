    ...
    ChainSpec: EthereumHardforks + Send + Sync + 'static,
{
    ...
    fn maybe_store_valid_bal(
        ...
        if let Err(err) = self.inner.bal_store.insert(block_hash, block_number, bal) {
            warn!(
                target: "rpc::engine",
                %err,
        ...
        PayloadT::ExecutionData: ExecutionPayload,
    {
        ...
        self.maybe_store_valid_bal(block_hash, block_number, bal, &status);
        Ok(status)
    }
...
mod tests {
    ...
    use reth_provider::{test_utils::MockEthProvider, RecentBalStore};
    use reth_tasks::Runtime;
    ...
    mod get_payload_bodies {
        ...
        async fn returns_payload_bodies_v2_with_bal_store_data() {
            let mut rng = generators::rng();
            let bal_store = BalStoreHandle::new(RecentBalStore::default());
