...
/// Default number of recent BALs retained in memory.
const DEFAULT_RECENT_BAL_CAPACITY: usize = 1024;

/// In-memory BAL store for recently validated blocks.
///
/// This is intended as a transient hot-path tier. It keeps the most recent BALs available after a
/// block is validated, before a future persistent backend takes over canonical history retention.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn evicts_oldest_number_first() {
        ...
        assert_eq!(store.get_by_range(20, 2).unwrap().len(), 2);
    }
